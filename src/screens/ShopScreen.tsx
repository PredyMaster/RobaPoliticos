...
> = {
  weapon: {
    shell: 'linear-gradient(180deg, rgba(142,64,24,0.94) 0%, rgba(93,31,13,0.98) 100%)',
    border: 'rgba(255, 150, 74, 0.72)',
    banner: 'linear-gradient(180deg, #a84c1f 0%, #713115 100%)',
    shadow: 'rgba(117, 44, 11, 0.45)',
  },
  hand: {
    shell: 'linear-gradient(180deg, rgba(10,88,152,0.94) 0%, rgba(7,59,112,0.98) 100%)',
    border: 'rgba(74, 182, 255, 0.68)',
    banner: 'linear-gradient(180deg, #0d75c2 0%, #08487d 100%)',
    shadow: 'rgba(9, 79, 147, 0.42)',
  },
  box: {
    shell: 'linear-gradient(180deg, rgba(41,104,18,0.94) 0%, rgba(22,70,15,0.98) 100%)',
...
export function ShopView({ embedded = false, onBack }: Required<ShopScreenProps>) {
  ...
  const [selected, setSelected] = useState<ShopSelection>(() => ({
    category: 'weapon',
    id: normalizeCatalogId('weapon', equipment?.equippedWeaponId ?? SHOP_WEAPONS[0].id),
  }))
    ...
    >
      ...
      >
        ...
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: layout.compact ? '1fr' : 'minmax(0, 1fr) 430px',
        ...
        >
          ...
          <DetailPanel item={selectedItem} compact={layout.compact} short={layout.short} />
        </div>
      </div>
    </div>
  )
    ...
    >
      <div
        style={{
          ...
          borderRadius: 24,
          border: PANEL_BORDER,
          background: 'linear-gradient(180deg, rgba(7,39,68,0.98) 0%, rgba(3,24,41,0.98) 100%)',
          boxShadow: 'inset 0 0 0 2px rgba(0,0,0,0.28), 0 14px 40px rgba(0,0,0,0.28)',
        }}
      >
        <MoneyStackIcon />
      ...
      >
        {items.map((item) => (
          <ShopTile
            ...
            item={item}
            selected={selected.category === item.category && selected.id === item.id}
...
function DetailPanel({
  ...
  async function handleAction() {
    ...
    if (status === 'owned') {
      ...
      setFeedback(result.ok ? 'Equipado correctamente.' : formatEquipError(result.error, item.category))
      setBusy(false)
      return
    }
    ...
    setFeedback(
      result.ok ? 'Comprado correctamente.' : formatPurchaseError(result.error, item.category),
    )
    setBusy(false)
  }

  const actionLabel = getActionLabel(status, busy, item.price, canBuy, item.unlockLevel)
  ...
  return (
    <aside
      style={{
        ...
        padding: compact ? '18px 18px 20px' : short ? '18px 18px 18px' : '22px 20px 20px',
        borderRadius: 24,
        border: PANEL_BORDER,
        background: 'linear-gradient(180deg, rgba(3,30,52,0.98) 0%, rgba(2,20,36,0.98) 100%)',
        boxShadow: 'inset 0 0 0 2px rgba(0,0,0,0.3), 0 22px 42px rgba(0,0,0,0.34)',
        display: 'flex',
        flexDirection: 'column',
      }}
    >
      ...
      <div style={{ marginTop: short ? 14 : 18, display: 'flex', flexDirection: 'column', gap: short ? 10 : 12 }}>
        {stats.map((stat) => (
      ...
      <div
        style={{
          ...
          background: 'linear-gradient(90deg, rgba(54,120,170,0), rgba(54,120,170,0.72), rgba(54,120,170,0))',
        }}
      />
      ...
      {feedback && (
        <p
          style={{
            margin: '14px 0 0',
            color: feedback.startsWith('Comprado') || feedback.startsWith('Equipado')
              ? '#9ce60e'
              : '#ff9b9b',
            fontSize: 13,
            fontWeight: 700,
      ...
      >
        <span style={{ fontSize: short ? 34 : 40, lineHeight: 1 }}>🛒</span>
      ...
      >
        <CoinIcon size={24} />
        <span>{Math.max(0, item.price - coins) > 0 ? formatCoins(item.price - coins) : 0}</span>
      </div>
    </aside>
  )
}
  ...
  return (
    <span
      ...
      style={{
        ...
        boxShadow: 'inset 0 0 0 2px rgba(255,255,255,0.2), 0 4px 10px rgba(0,0,0,0.26)',
        color: '#8c5200',
...
function MoneyStackIcon() {
  return (
    <div style={{ position: 'relative', width: 108, height: 74, flexShrink: 0 }}>
...
function useShopLayout(widthBreakpoint = 1220, heightBreakpoint = 860): ShopLayout {
...
function useItemStatus(
  ...
  if (equippedId && normalizeCatalogId(itemType, equippedId) === itemId) return 'equipped'
...
function getCatalogItem(category: ShopItemType, id: string): ShopCatalogItem {
  return ITEM_MAP.get(`${category}:${normalizeCatalogId(category, id)}`) ?? SHOP_WEAPONS[0]
}
...
const backButtonStyle: CSSProperties = {
  ...
  boxShadow: 'inset 0 0 0 2px rgba(255,255,255,0.16), 0 16px 32px rgba(0,0,0,0.24)',
  display: 'inline-flex',
  alignItems: 'center',
  justifyContent: 'center',
  gap: 18,
...
const actionButtonStyle: CSSProperties = {
  ...
  justifyContent: 'center',
  gap: 18,
  boxShadow: 'inset 0 0 0 2px rgba(255,255,255,0.14), 0 18px 36px rgba(0,0,0,0.26)',
}
